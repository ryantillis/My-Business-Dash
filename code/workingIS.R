...
server <- function(input, output) {
       ...
       invdollendyear <- reactive({c(desiredinv()[1]*pcostinf()[1],
       ...
       #SG & A
       sga <- reactive({
              sga <- c(rep(input$B19,8)+input$B20*sales_vol())
              sga[8] <- input$B40+input$B41*sales_vol()[8]
              sga
       })
       
       #Note: Need to add k93 to this portion for recursive asset changes
       ...
       output$cash <- renderTable({
              cash <- rbind(ni(),Book_dep())
              colnames(cash) <- c("Year 1","Year 2","Year 3","Year 4","Year 5","Year 6","Year 7","Year 8")
              row.names(cash) <- c("NET INCOME", "ADD DEPRECIATION","GROSS MARGIN", "- Change Accts Rec", "- Change Inv", "+ Change in Accts Pay", "+Change in Wage paya", "Other", "Cash from Operations", "PPE Investment", "PPE Disposal")
